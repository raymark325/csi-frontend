<template>
  <div class="q-pa-lg">
    ...
    <q-dialog v-model="showDeployDialog" persistent>
      <q-card class="glass-q-card" style="width: 500px; max-width: 90vw;">
        <q-card-section>
          <div class="text-h6 text-primary font-weight-bold">Deploy Lesson</div>
          <p class="text-caption text-muted">Select the sections you want to post this master lesson to.</p>
        </q-card-section>
        ...
        <q-card-actions align="right" class="q-pb-md q-pr-md">
          ...
          <q-btn label="Deploy Lesson" color="primary" rounded unelevated @click="handleDeploy" :loading="isDeploying" :disable="!selectedSectionIds.length" />
        </q-card-actions>
      </q-card>
    </q-dialog>
...
const handleDeploy = async () => {
  ...
  try {
    ...
    $q.notify({
      color: 'positive',
      message: 'Lesson deployed to selected sections!',
      icon: 'check_circle'
    });
    ...
    $q.notify({
      color: 'negative',
      message: err.message || 'Failed to deploy lesson',
      icon: 'error'
    });
  } finally {
